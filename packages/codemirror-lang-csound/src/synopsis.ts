...
import { getCsoundHoverInfo } from "./hover.js"
import { analyzeCsoundSemanticLine } from "./semantic.js"
...
function opcodeAtSelection(state: EditorState): string | undefined {
  ...
  while (node) {
    if (node.name === "FunctionCallExpr" || node.name === "ScoreFunctionCallExpr") {
      const callee = node.getChild("FunctionCallee") ?? node.getChild("ScoreFunctionCallee")
      if (callee) return state.sliceDoc(callee.from, callee.to).trim()
    }
    if (node.name === "OrcGenericLine") {
      const text = state.sliceDoc(node.from, node.to)
      const span = analyzeCsoundSemanticLine(text, { documentText: state.doc.toString() })
        .find(span => span.kind === "builtInOpcode" || span.kind === "userOpcode")
...
export function csoundSynopsis(): Extension {
  return showPanel.of(view => {
    ...
    function refresh() {
      ...
      void getCsoundHoverInfo(name, { documentText: view.state.doc.toString() }).then(info => {
        ...
        const signature = info.signatures.find(signature => rate && signature.outTypes === rate) ?? info.signatures[0]
        const syntax = info.syntax?.find(line => !rate || line.trim().startsWith(rate)) ?? info.syntax?.[0]
        const text = syntax ?? (signature
        ...
        if (!destroyed && current === request) dom.replaceChildren()
      })
    }
    ...
    return {
      ...
      top: false,
      update(update) {
        if (update.docChanged || update.selectionSet || syntaxTree(update.state) !== syntaxTree(update.startState)) refresh()
      },
      destroy() {
        destroyed = true
        request++
